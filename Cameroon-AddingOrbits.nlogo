...
to setup
  ...
  ;determine a set of randomly chosen points where the epidemic will start
  let num-points 1
  let epi-start-patches n-of num-points (land with [any? sedentary-herds-here])
  set epi-start-locations [ ]
  ask epi-start-patches [ set epi-start-locations lput (list pxcor pycor) epi-start-locations ]
...
to start-infection
  if (epidemic-start-location = "single") [
    let x-start int infection-start-location
    let y-start round (100 * (infection-start-location mod int infection-start-location))
  ...
  if (epidemic-start-location = "multiple") [
    let start-patches (patch-set (patch 44 74) (patch 68 5) (patch 40 22) (patch 18 23))
    ask start-patches [
      set pcolor red
      set p-sus (p-sus - 1)
      set p-inf (p-inf + 1)
...
to setup-patches
  ...
  file-open "map.txt"
  let map-matrix matrix:from-row-list file-read
  file-close
...
to save-SIR-data
  ask land [
    matrix:set S-data pxcor pycor precision (p-sus) 4
    matrix:set I-data pxcor pycor precision (p-inf) 4
    matrix:set R-data pxcor pycor precision (p-rec) 4
  ]
  ...
  if (any? mobile-herds) [
    foreach n-values length mobile-list [?] [
      ask item ? mobile-list [
        matrix:set mobile-S ? 2 (precision sus 4) matrix:set mobile-S ? 3 pxcor matrix:set mobile-S ? 4 pycor
        matrix:set mobile-I ? 2 (precision inf 4) matrix:set mobile-I ? 3 pxcor matrix:set mobile-I ? 4 pycor
        matrix:set mobile-R ? 2 (precision rec 4) matrix:set mobile-R ? 3 pxcor matrix:set mobile-R ? 4 pycor
      ]
    ]
  ]
...
to setup-zones
  ;; all the zone patches are set here, but only the outline will be drawn in...
  set cold-dry lput (patches with [ pxcor >= 52 and pxcor <= 58 and pycor >= 19 and pycor <= 23 and p-type = 0]) [ ]
  set cold-dry lput (patches with [ pxcor >= 45 and pxcor <= 64 and pycor >= 34 and pycor <= 52 and p-type = 0]) cold-dry
  set cold-dry lput (patches with [ pxcor >= 56 and pxcor <= 60 and pycor >= 56 and pycor <= 60 and p-type = 0]) cold-dry
  
  draw-box 52 58 19 23 (blue - 1)
  draw-box 45 64 34 52 (blue - 1)
  draw-box 56 60 56 60 (blue - 1)

  set hot-dry lput (patches with [ pxcor >= 51 and pxcor <= 63 and pycor >= 16 and pycor <= 35 and p-type = 0]) [ ]
  set hot-dry lput (patches with [ pxcor >= 51 and pxcor <= 62 and pycor >= 37 and pycor <= 52 and p-type = 0]) hot-dry
  set hot-dry lput (patches with [ pxcor >= 55 and pxcor <= 61 and pycor >= 53 and pycor <= 68 and p-type = 0]) hot-dry
    
  draw-box 51 63 16 35 (red)
  draw-box 51 62 37 52 (red)
  draw-box 55 61 53 68 (red)

  set rainy lput (patches with [ pxcor >= 28 and pxcor <= 48 and pycor >= 6 and pycor <= 19  and p-type = 0]) [ ]
  set rainy lput (patches with [ pxcor >= 39 and pxcor <= 42 and pycor >= 25 and pycor <= 29 and p-type = 0]) rainy
  set rainy lput (patches with [ pxcor >= 29 and pxcor <= 36 and pycor >= 34 and pycor <= 38 and p-type = 0]) rainy

  draw-box 28 48 6 19  (green - 1)
  draw-box 39 42 25 29 (green - 1)
  draw-box 29 36 34 38 (green - 1)
  
  set transition patches with [ (pxcor >= 59 and pxcor <= 63 and pycor >= 10 and pycor <= 13) ] 
  draw-box 60 63 10 13 (yellow - 2)
...
to write-run-data
  let filename "f"
  if (epidemic-start-location = "single")   [set filename (word "data-4316-orbit" orbit-to-include "-Run" behaviorspace-run-number "-2.csv")]
  if (epidemic-start-location = "multiple") [set filename (word "data-multiple-orbit" orbit-to-include "-Run" behaviorspace-run-number "-2.csv")]
  if (epidemic-start-location = "mobile")   [set filename (word "data-mobile-orbit" orbit-to-include "-Run" behaviorspace-run-number "-2.csv")]
  if (file-exists? filename) [file-delete filename]
  file-open filename  
...
@#$#@#$#@
<experiments>
  <experiment name="EffectOfOrbits" repetitions="20" runMetricsEveryStep="false">
