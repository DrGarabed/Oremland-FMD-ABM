...
to setup
  ...
  set-patch-size 7 * (80 / map-size)

  setup-sedentary-herds
  
  set color-list-string ["red" "orange" "yellow" "blue" "green" "pink" "cyan" "magenta" "brown" "gray"]
  set color-list [ red orange yellow blue green pink cyan magenta brown gray]
end
...
to fill-in
  if (mouse-down? and member? (patch mouse-xcor mouse-ycor) land) [
    set current-color item (position region-color color-list-string) color-list
    
    let pcol [pcolor] of patch mouse-xcor mouse-ycor
    if (pcol = current-color) [ set current-color white ]
    
    ;if (brush-size = 2) [ set patches-to-color (patch-set patches-to-color (patch-at 0 1) (patch-at 1 0) (patch-at 1 1)) ]
    
    ask patch mouse-xcor mouse-ycor [
      set pcolor current-color
      if brush-size = 3 [ask neighbors with [member? self land] [set pcolor current-color]]
      wait 0.15
    ]
  ]
end
...
to setup-sedentary-herds
  ...
  (foreach x-loc y-loc [
    create-sedentary-herds 1 [
      set shape "dot"      
      set size 0.4 * map-size / 80
      set color black
      ;print ?2
      setxy (?1 * max-pxcor / 79) (?2 * max-pycor / 89)
    ]
  ])
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
to setup-patches
  let mapfilename (word "map" map-size ".txt") 

  file-open (mapfilename)
  let map-matrix matrix:from-row-list file-read
  file-close
 
  let dims matrix:dimensions map-matrix

  resize-world 0 (item 1 dims - 1) 0 (item 0 dims - 1)
...
to save-configuration
  ask land [ set r-number (1 + position pcolor color-list) ]
  let old-vals sort remove-duplicates [r-number] of land
  ask land [ set r-number (1 + position r-number old-vals) ]
    
  set region-map matrix:make-constant world-width world-height 0  
  ask land [ matrix:set region-map pxcor pycor r-number ]
  
  if (file-exists? filename) [file-delete filename]
  file-open filename
  file-print matrix:to-row-list region-map
  file-close
  export-view word (substring filename 0 (length filename - 4)) ".png"
end

to save-configuration-csv
  ask land [ set r-number (1 + position pcolor color-list) ]
  let old-vals sort remove-duplicates [r-number] of land
  ask land [ set r-number (1 + position r-number old-vals) ]
  ...
  let final-filename (word "Size" map-size filename)
  
  if (file-exists? final-filename) [file-delete final-filename]  
  csv:to-file final-filename matrix:to-row-list region-map
  
;  if (file-exists? filename) [file-delete filename]
;  file-open filename
;  file-print matrix:to-row-list region-map
;  file-close
  export-view word (substring final-filename 0 (length final-filename - 4)) ".png"
end

to reload-regionmap
  let r-map csv:from-file "SixRegions.csv"
  
  let x 0
  let y 0
  while [x < length r-map] [
    set y 0
    while [y < length item 0 r-map] [
      ask patch x y [set r-number item y item x r-map]
      set y y + 1
    ]
    set x x + 1
  ]
  
  let c-list [red orange yellow blue green pink]
  
  ask land [
    set pcolor item (r-number - 1) c-list
  ]
  
  print length r-map
  print length item 0 r-map
...
region-color
region-color
"red" "orange" "yellow" "blue" "green" "pink" "cyan" "magenta" "brown" "gray"
0
...
@#$#@#$#@
<experiments>
  <experiment name="EffectOfOrbits" repetitions="5" runMetricsEveryStep="false">
