...
to setup-mobile-herds
  ...
  create-mobile-herds (initial-mobile-herds) [
    ...
    let eligible-orbits remove (orbit-to-remove - 1) n-values length movement-patterns [?]
    set trajectory item (one-of eligible-orbits) movement-patterns
  ...
  ;show remove-duplicates sort [my-orbit] of mobile-herds
end
...
to write-run-data
  ...
  let filename "f"
  if (epidemic-start-location = "single")   [set filename (word "size" width "-data-4316-RemOrbit" orbit-to-remove "-Run" int (behaviorspace-run-number mod 50) ".csv")]
  if (epidemic-start-location = "multiple") [set filename (word "size" width "-data-multiple-RemOrbit" orbit-to-remove "-Run" int (behaviorspace-run-number mod 50) ".csv")]
  if (epidemic-start-location = "mobile")   [set filename (word "size" width "-data-mobile-RemOrbit" orbit-to-remove "-Run" int (behaviorspace-run-number mod 50) ".csv")]
  if (file-exists? filename) [file-delete filename]
  file-open filename  
...
to write-run-data2
  let filename "f"
  if (epidemic-start-location = "single")   [set filename (word "size" width "-data-4316-RemOrbit" orbit-to-remove "-")]
  if (epidemic-start-location = "multiple") [set filename (word "size" width "-data-multiple-RemOrbit" orbit-to-remove "-")]
...
@#$#@#$#@
<experiments>
  <experiment name="FullMobility" repetitions="50" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    ...
    <enumeratedValueSet variable="orbit-to-remove">
      <value value="0"/>
    </enumeratedValueSet>
